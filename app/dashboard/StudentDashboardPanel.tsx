...
type Announcement = { id: string; type: string; title: string; body: string | null; startDate: string | null; endDate: string | null; createdAt: string };
          ...
          <ul className="divide-y divide-slate-100 dark:divide-slate-800">
            {announcements.map((a) => (
              <li key={a.id} className="px-4 sm:px-5 py-4">
                <span className={`inline-block text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded mb-2 ${a.type === "ETKINLIK" ? "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"}`}>
                  {a.type === "ETKINLIK" ? "Etkinlik" : "Duyuru"}
                </span>
                <p className="font-medium text-slate-800 dark:text-slate-100">{a.title}</p>
                {a.body && <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 line-clamp-2">{a.body}</p>}
                {(a.startDate || a.endDate) && (
                  <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">
                    {a.startDate && a.endDate ? `${a.startDate} – ${a.endDate}` : a.startDate ?? a.endDate}
                  </p>
                )}
              </li>
            ))}
